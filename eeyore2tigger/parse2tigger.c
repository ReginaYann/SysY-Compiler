...
void analyzeLabel(Expr* expr)
{
	int num = atoi(expr->op);
	CPY(actTab[expr->index], actTab[expr->index+1], sizeof(actTab[0]));
	// jump from former line to this line
	if(gotoIndex[num] < expr->index) return;
	// jump from latter line to this line
...
int suitableReg(int * pstackSize, int contra1, int contra2)
{
	...
	/*if (regUsage[i]==-1){
		int tmpupd=	resultIndex;
		++resultIndex;
		char sta[5] = {0};
		sprintf(sta, "%d", recordimme[i]);
		makeEntry(_rload, tmpupd, sta, reg2string(i), NULL, NULL);
		return i;
	}*/
	...
	int upd_i = resultIndex;
	++resultIndex;
	makeEntry(_rstor, upd_i, temp, sta, NULL, VarList[regUsage[lastendind]]->name);
	return lastendind;
}

const char* turnToUse(int index, const char* varName, int* pstackSize, short leftside, int contra1, int contra2,short canimme)
{
	char temp[5] = {0};
	if(varName[0] == 'p')
	{
...
//fprintf(stdout,"imme= %d\n",imme);
//if (imme==false)
//fprintf(stdout,"regindex[%d]= %d\n",varIndex,regIndex[varIndex]);
	if(imme == false && regIndex[varIndex] >= 0 && regIndex[varIndex] < 20 && regIndex[varIndex]!=contra1 && regIndex[varIndex]!=contra2)
	{
		strcpy(temp, reg2string(regIndex[varIndex]));
		return strdup(temp);
	}
//fprintf(stdout,"can reach here\n");
	if(imme == true && canimme==true && varName[0]!='-')
	{
		/*int upd_i = resultIndex;
		++resultIndex;
	...
	if (imme==true && canimme==false)
	{
		int upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rassi, upd_i, reg2string(regnum), varName, NULL, NULL);
		regUsage[regnum] = -1;
		//if (ResList[upd_i-1]->type==_rstor && EQ(ResList[upd_i-1]->op,reg2string(regnum))){
			//regIndex[]=0-atoi(ResList[upd_i-1]->src1);
		//}
		recordimme[regnum]=(*pstackSize)-1;
		return strdup(temp);
	}
	else if (imme==true && varName[0]=='-')
	{
		int upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rope2, upd_i, "-", "x0", varName+1, temp);
		regUsage[regnum] = -1;
		return strdup(temp);
	}
	...
	else if (VarList[varIndex]->global==true&& leftside == false){
		int upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rload, upd_i, VarList[varIndex]->globalname, temp, NULL, NULL);
	}
	/*else if(VarList[varIndex]->array == false&&leftside==true){
		int upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rload, upd_i, VarList[varIndex]->globalname, temp, NULL, NULL);
	}*/
//fprintf(stdout,"VarList[varIndex]->array %d    leftside=%d\n",VarList[varIndex]->array,leftside);
	if(imme == false)
	{
	...
	int tmpupd=resultIndex;
	++resultIndex;
	makeEntry(_rprint, tmpupd, VarList[varIndex]->name,  reg2string(regIndex[varIndex]), NULL, NULL);
	return strdup(temp);
}

const char* turnParamToUse(int index, const char* varName, int* pstackSize, int StackPlace)
{
	if(varName[0] != 'p') return turnToUse(index, varName, pstackSize, false, -1, -1,false);
...
void handleDecl(int index, int* pstackSize)
{
	...
	VarList[varIndex]->globalname=strdup(sta);
	//fprintf(stdout,"here declare: %s %s\n",varName,VarList[varIndex]->globalname);

	//not array
	if(ExprList[index]->dst == NULL){
		//regIndex[varIndex]=-(*pstackSize);
		(*pstackSize) += 1;
	}
	//array
	else
...
{
	const char* src1 = turnToUse(index, ExprList[index]->src1, pstackSize, false, -1, -1,false);
	const char* src2 = turnToUse(index, ExprList[index]->src2, pstackSize, false, string2reg(src1), -1,true);
	const char* dst = turnToUse(index, ExprList[index]->dst, pstackSize, true, string2reg(src1), string2reg(src2),false);
	int upd_i = resultIndex;
	++resultIndex;
...
{
	const char* src1 = turnToUse(index, ExprList[index]->src1, pstackSize, false, -1, -1,false);
	const char* dst = turnToUse(index, ExprList[index]->dst, pstackSize, true, string2reg(src1), -1,false);
	int upd_i = resultIndex;
	++resultIndex;
...
void handleAssi(int index, int* pstackSize)
{
	...
	{
		// rightvalue		
		const char* src1 = turnToUse(index, ExprList[index]->op, pstackSize, false, -1, -1,true);
		// varname
		const char* op = turnToUse(index, ExprList[index]->src1, pstackSize, true, string2reg(src1), -1,false);
		int upd_i = resultIndex;
		++resultIndex;
	...
	// var[index] = rightvalue
	else if(EQ(ExprList[index]->src2,"1"))
	{
		// varname
		const char* op = turnToUse(index, ExprList[index]->src1, pstackSize, false,-1, -1,false);
		// index
		const char* src2 = turnToUse(index, ExprList[index]->dst, pstackSize, false, string2reg(op), -1,true);
		// if index not a imme num
		//if (ExprList[index]->dst[0]=='T' || ExprList[index]->dst[0]=='t'){
		const char* dst = reg2string(suitableReg(pstackSize, string2reg(src2), string2reg(op)));
		int upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rope2, upd_i, "+", op, src2, dst);
		//}
		//rightvalue
		const char* src1 = turnToUse(index, ExprList[index]->op, pstackSize, false, string2reg(dst),-1,false);
		upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rassi, upd_i, dst, src1, "0", NULL);	
		disableReg(index);	
	}
	// var = var[index]
	else if (EQ(ExprList[index]->src2,"2"))
	{
		//const char* op = "+";
		// varname(right of the asg)
		const char* src1 = turnToUse(index, ExprList[index]->op, pstackSize, false, -1, -1,false);
		// index
		const char* dst = turnToUse(index, ExprList[index]->dst, pstackSize, false, string2reg(src1), -1,true);
		const char* temp = reg2string(suitableReg(pstackSize, string2reg(src1), string2reg(dst)));
		int upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rope2, upd_i, "+", src1, dst, temp);
		// varname(left of the asg)
		const char* op = turnToUse(index, ExprList[index]->src1, pstackSize, false, string2reg(dst),string2reg(src1) ,false);
		upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rassi, upd_i, op, temp, NULL, "0");
		disableReg(index);
	}
	// varname [index] = -IMM
	else{
		...
		makeEntry(_rope2, upd_i, "+", op, src2, dst);
		//rightvalue (minus)
		const char* src1 = reg2string(suitableReg(pstackSize, string2reg(dst), -1));
		upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rope2,upd_i,"-","x0",ExprList[index]->op,src1);
		upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rassi, upd_i, dst, src1, "0", NULL);	
		disableReg(index);	
...
void handleIfbr(int index, int* pstackSize)
{
	...
	const char* src1 = turnToUse(index, ExprList[index]->src1, pstackSize, false, -1, -1,false);
	const char* src2 = turnToUse(index, ExprList[index]->src2, pstackSize, false, string2reg(src1), -1,false);

	int upd_i = resultIndex;
	++resultIndex;
	makeEntry(_rifbr, upd_i, ExprList[index]->op , src1, src2, ExprList[index]->dst);

	/*if (ExprList[index]->src2[0]!='t'&&ExprList[index]->src2[0]!='T'&&ResList[upd_i-2]->type==_rstor){
		upd_i = resultIndex;
		++resultIndex;
		makeEntry(_rload, upd_i,ResList[upd_i-3]->src1 , ResList[upd_i-3]->op, NULL, NULL);
	}*/
	...
	while(ExprList[pre_i]->type != _ends)
	{
		switch(ExprList[pre_i]->type)
		{
				...
				if(paramNow == 0)
				{
					for(j = 0; j < 7; ++j)
						if(funcSaveReg[j] == -1 && regUsage[j] >= 0)
						{
							...
							makeEntry(_rstor, upd_i, reg2string(j), sta, NULL, "parastore t");
						}
					...
					for(j = 0; j<funcPara; ++j)
					{
						...
						makeEntry(_rstor, upd_i, temp1, temp2, NULL, "parastore a");
					}
					if(funcPara == 0)
					{
						int upd_i = resultIndex;
						...
						char temp2[5] = {0};
						sprintf(temp2, "%d", stackSize++);
						makeEntry(_rstor, upd_i, temp1, temp2, NULL, "parastore a0");
					}
				}
			...
			case _call:
			{
				if(paramNow == 0)
				{
					for(j = 0; j < 7; ++j)
						if(funcSaveReg[j] == -1 && regUsage[j] >= 0)
						{
							...
							char sta[5] = {0};
							sprintf(sta, "%d", funcSaveReg[j]);
							makeEntry(_rstor, upd_i, reg2string(j), sta, NULL, "callstore t");
						}
					startStack = stackSize;
					int upd_i = resultIndex;
					++resultIndex;
					...
					char temp2[5] = {0};
					sprintf(temp2, "%d", stackSize++);
					makeEntry(_rstor, upd_i, temp1, temp2, NULL, "callstore t0");
				}
				int upd_i = resultIndex;
				++resultIndex;
				...
				{
					const char* op = turnToUse(pre_i, ExprList[pre_i]->op, &stackSize, true, -1, -1,false);
					upd_i = resultIndex;
					resultIndex++;
			...
			case _retu:
			{
				...
				if (ExprList[pre_i]->op!=NULL){
					const char* src1 = turnToUse(pre_i, ExprList[pre_i]->op, &stackSize, false, -1, -1,true);
					upd_i = resultIndex;
					++resultIndex;
	...
	for(i = 0; i < resultIndex; ++i)
	{
		switch(ResList[i]->type)
		{
			...
			case _rends:
				...
				break;
			case _rprint:
				fprintf(stdout, "	register: put var %s to reg %s.\n", ResList[i]->op,ResList[i]->src1);
				break;
		}
	}
	return;
